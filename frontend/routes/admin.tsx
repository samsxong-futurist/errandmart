...
import type { InternalField } from "@/libs/csv";
import {
  adminOverview,
  importProducts,
  type AdminOverviewProduct,
  type AdminOverviewRun,
} from "@/libs/api";
...
export default function AdminPage() {
  ...
  const [overview, setOverview] = useState<{
    ...
    runs: AdminOverviewRun[];
  } | null>(null);

  const [fileName, setFileName] = useState("");
  const [headers, setHeaders] = useState<string[]>([]);
  const [rows, setRows] = useState<string[][]>([]);
  const [mapping, setMapping] = useState<Record<InternalField, string>>({
    product_name: "",
    price: "",
    image_url: "",
    sku: "",
  ...
  const preparedRows = useMemo(() => {
    if (!mapping.product_name || !mapping.price) return [];
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Button
                variant="brand"
                size="lg"
                disabled={preparedRows.length === 0 || loading}
                onClick={runImport}
              >
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="soft"
              size="sm"
              disabled={!overview || overview.products.length === 0}
              onClick={downloadCurrentCatalogue}
            >
