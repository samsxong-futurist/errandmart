...
export function FloatingOrderBar({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40 p-4">
      <button
        type="button"
        onClick={onOpen}
        className="animate-rise-in pointer-events-auto mx-auto flex w-full max-w-md items-center justify-between gap-3 rounded-2xl bg-brand px-5 py-4 text-brand-foreground shadow-float transition-transform active:scale-[0.99]"
      >
