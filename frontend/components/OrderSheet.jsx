...
export function OrderSheet({ open, onOpenChange, }) {
    ...
    return (<Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="flex h-[88vh] flex-col rounded-t-3xl p-0 sm:max-w-lg">
        ...
        {lines.length === 0 ? (<div className="flex flex-1 flex-col items-center justify-center gap-3 px-8 text-center">
            ...
            <div className="flex-1 divide-y divide-border overflow-y-auto px-5">
              {lines.map((line) => (<div key={line.product.id} className="flex items-start gap-3 py-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-1">
                      <Button variant="soft" size="icon" className="rounded-lg" onClick={() => setQuantity(line.product.id, line.quantity - 1)} aria-label="Decrease quantity">
                        <Minus />
                      </Button>
                      <span className="w-8 text-center text-sm font-semibold">{line.quantity}</span>
                      <Button variant="soft" size="icon" className="rounded-lg" onClick={() => setQuantity(line.product.id, line.quantity + 1)} aria-label="Increase quantity">
                        <Plus />
                      </Button>
                      <Button variant="ghost" size="icon" className="ml-1 text-muted-foreground hover:text-destructive" onClick={() => remove(line.product.id)} aria-label={`Remove ${line.product.product_name}`}>
                        <Trash2 />
                      </Button>
                    </div>
                  </div>
              ...
              <div className="py-4">
                ...
                <Textarea id="order-note" value={note} onChange={(e) => setNote(e.target.value)} placeholder="e.g. deliver to Ikeja GRA this evening" className="mt-1 resize-none rounded-xl" rows={2}/>
              </div>
            </div>
