import { supabaseAdmin } from "./integration/supabase/client.server.js";
async function runImport(input) {
  const { data: existing, error: readError } = await supabaseAdmin.from("products").select("id, sku, product_name");
  ...
  if (input.deactivateMissing) {
    ...
    if (missing.length) {
      const { error } = await supabaseAdmin.from("products").update({ is_active: false }).in("id", missing);
      if (error) throw new Error(error.message);
...
async function getOverview() {
  const [products, runs] = await Promise.all([
    supabaseAdmin.from("products").select("id, product_name, price, image_url, sku, category, updated_at, is_active").order("updated_at", { ascending: false }).limit(200),
    supabaseAdmin.from("import_runs").select("*").order("created_at", { ascending: false }).limit(5)
  ]);
